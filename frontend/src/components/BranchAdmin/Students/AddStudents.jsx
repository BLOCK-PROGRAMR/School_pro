...
const AddStudents = () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (studentCountData.success) {
          ...
          // const newId = `${yearSuffix}${paddedCount}`;
          // console.log("newId", newId);

          setstdcount(nextCount);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Failed to add student: ${res.statusText}`);
      }
      ...
      if (result.success) {
        ...
        const nextCount = stdcount + 1;
        const paddedCount = String(nextCount).padStart(4, '0');
        // const newId = `${ysuffix}${paddedCount}`;
