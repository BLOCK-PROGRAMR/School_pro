    ...
    try {
      ...
      const receiptResponse = await fetch(Allapi.addReciepts.url, {
        ...
        body: JSON.stringify({
          ...
          feeLedger: paymentDetails
            .filter((fee) => fee.enteredAmount > 0) // Only include fees with entered amounts
            .map((fee) => ({
              name: fee.name,
              amount: fee.enteredAmount,
              terms: selectedTerm ? selectedTerm.split('-')[1] : "1" // Use selected term or default to 1
            })),
          totalAmount: paymentDetails
            .filter((fee) => fee.enteredAmount > 0)
            .reduce((sum, fee) => sum + fee.enteredAmount, 0)
        }),
      }); 
